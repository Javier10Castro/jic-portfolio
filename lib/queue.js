...
class BackgroundQueue {
  ...
  enqueue({ handler, req, label }) {
    ...
    log.structured(req, { stage: 'queue.enqueue', status: 'ok', queueId: id, endpoint: label, position, depth: this.queue.length + this.active });
  ...
  async _process(item) {
    try {
      ...
      const queueCompletionStage = itemStatus === 'completed' ? 'queue.complete' : 'queue.fail';
      log.structured(req, { stage: queueCompletionStage, status: itemStatus, queueId: id, endpoint: label, attempts: itemStatus === 'failed' ? 4 : undefined });
