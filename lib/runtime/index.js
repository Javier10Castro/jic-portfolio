...
async function createProject(workspaceId, userId, name, formData, workspaceSlug) {
  ...
  const project = await createProjectRow(ctx.workspace.id, ctx.user.id, name, slug, formData);
...
async function runPipeline(projectId, workspaceId, executionId, formData, inputType, workspaceSlug, userId) {
  assertUUID(projectId, 'project_id');
  assertUUID(executionId, 'execution_id');
  const ctx = await resolveContext({ workspace_slug: workspaceSlug, workspace_id: workspaceId, user_id: userId });
  const resolvedWsId = ctx.workspace.id;
  const project = await getProjectById(projectId, resolvedWsId);
  ...
  try {
    ...
    await emitEvent('plan.completed', projectId, resolvedWsId, executionId, { categories: Object.keys(planResult.result.project || {}).length, ir: planResult.result });
  } catch (err) {
    await failPipeline(projectId, executionId, 'plan', err);
    await emitEvent('pipeline.failed', projectId, resolvedWsId, executionId, { step: 'plan', error: err.message });
    throw err;
  }
  ...
  try {
    ...
    await emitEvent('design.completed', projectId, resolvedWsId, executionId, { theme: dsResult.result.mapping.theme, palette_size: dsResult.result.colors.palette.length, design_system: dsResult.result });
  } catch (err) {
    await failPipeline(projectId, executionId, 'design_system', err);
    await emitEvent('pipeline.failed', projectId, resolvedWsId, executionId, { step: 'design_system', error: err.message });
    throw err;
  }
  ...
  try {
    ...
    await emitEvent('preview.generated', projectId, resolvedWsId, executionId, { preview_id: previewId, sections: previewResult.result.layout.sections.length, warnings_count: previewResult.result.warnings.length, warnings: previewResult.result.warnings });
  } catch (err) {
    await failPipeline(projectId, executionId, 'preview', err);
    await emitEvent('pipeline.failed', projectId, resolvedWsId, executionId, { step: 'preview', error: err.message });
    throw err;
  }
  ...
  try {
    ...
    await emitEvent('scoring.completed', projectId, resolvedWsId, executionId, { score: totalScore, passed, metrics, warnings_count: warnings.length, decision_id: decisionId, preview_id: previewId });
    ...
    await emitEvent('pipeline.failed', projectId, resolvedWsId, executionId, { step: 'scoring', error: err.message });
    throw err;
  }
...
async function approveProject(projectId, workspaceId, userId, workspaceSlug) {
  ...
  assertUUID(userId, 'user_id');
  const ctx = await resolveContext({ workspace_slug: workspaceSlug, workspace_id: workspaceId, user_id: userId });
  const project = await getProjectById(projectId, ctx.workspace.id);
  if (!project) throw makeError('NOT_FOUND', 'Project not found', 'project_id');
  ...
  try {
    ...
    await emitEvent('deployment.completed', projectId, ctx.workspace.id, executionId, { status: finalStatus, scaffold_files: (scaffoldResult.files || []).length, repo_url: deployResult.steps?.register?.repo_url || null });
