...
const EDGE_SOFT_LIMIT = parseInt(process.env.RL_EDGE_SOFT || '30');
const EDGE_HARD_LIMIT = parseInt(process.env.RL_EDGE_HARD || '60');
...
function slidingWindow(key, maxReqs, windowMs) {
  ...
  if (log.length >= maxReqs) {
    ...
    return { allowed: false, remaining: 0, limit: maxReqs, retryAfter, currentUsage: maxReqs };
  }
  log.push(now);
  return { allowed: true, remaining: maxReqs - log.length, limit: maxReqs, retryAfter: 0, currentUsage: log.length };
}
...
function edgeCheck(key) {
  ...
  if (!soft.allowed) {
    ...
    return {
      ...
      currentUsage: hard.limit - hard.remaining,
    };
  }
  if (!hard.allowed) {
...
function emailDedup(email, windowMs) {
  ...
  if (filtered.length > 0) {
    ...
    return { allowed: false, remaining: 0, limit: 1, retryAfter, currentUsage: filtered.length };
  }
  filtered.push(now);
  emailTimestamps.set(email, filtered);
  log.debugLog(null, 'emailDedup:ALLOW', { email: maskEmail(email) });
  return { allowed: true, remaining: 1, limit: 1, retryAfter: 0, currentUsage: 0 };
}
...
function timingCheck(body) {
  ...
  const now = Date.now();
  if (body.submittedAt > now + MAX_FUTURE_MS) {
    log.debugLog(null, 'timingCheck:BLOCK', { reason: 'future_timestamp', submittedAt: body.submittedAt, now });
    return { blocked: true, reason: 'INVALID_REQUEST' };
  }
  const elapsed = now - body.submittedAt;
  ...
  return { blocked: false, elapsedMs: elapsed, reason: 'ok' };
}
...
function getSnapshot() {
  return {
    ipEntries: ipLog.size,
    emailEntries: emailTimestamps.size,
    edgeSoftLimit: EDGE_SOFT_LIMIT,
    edgeHardLimit: EDGE_HARD_LIMIT,
    edgeWindowMs: EDGE_WINDOW_MS,
    emailDedupWindowMs: EMAIL_DEDUP_WINDOW_MS,
    instanceId: INSTANCE_ID,
  };
}

function getDetailedSnapshot() {
  const now = Date.now();
  let ipOldest = null;
  for (const log of ipLog.values()) {
    if (log.length > 0) {
      const first = log[0];
      if (ipOldest === null || first < ipOldest) ipOldest = first;
    }
  }
  let emailOldest = null;
  for (const timestamps of emailTimestamps.values()) {
    if (timestamps.length > 0) {
      const first = timestamps[0];
      if (emailOldest === null || first < emailOldest) emailOldest = first;
    }
  }
  const cutoffIp = now - EDGE_WINDOW_MS;
  const cutoffEmail = now - EMAIL_DEDUP_WINDOW_MS;
  return {
    ip: {
      entries: ipLog.size,
      oldestEntryAgeMs: ipOldest !== null ? Math.round(now - ipOldest) : 0,
      windowResetTime: new Date(cutoffIp + EDGE_WINDOW_MS).toISOString(),
      softLimit: EDGE_SOFT_LIMIT,
      hardLimit: EDGE_HARD_LIMIT,
      windowMs: EDGE_WINDOW_MS,
    },
    emailDedup: {
      entries: emailTimestamps.size,
      oldestEntryAgeMs: emailOldest !== null ? Math.round(now - emailOldest) : 0,
      windowResetTime: new Date(cutoffEmail + EMAIL_DEDUP_WINDOW_MS).toISOString(),
      limit: 1,
      windowMs: EMAIL_DEDUP_WINDOW_MS,
    },
    instanceId: INSTANCE_ID,
