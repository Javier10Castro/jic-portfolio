...
class ConversationEvents extends EventEmitter {
  ...
  emitQuestionsGenerated(conversationId, data) {
    const event = { type: 'conversation.questions.generated', conversationId, timestamp: new Date().toISOString(), data };
    this._eventLog.push(event);
    this.emit('conversation.questions.generated', event);
  }
