...
function recordQuestionHistory(context, questionResult) {
  if (!context.questionHistory) context.questionHistory = [];
  const entry = {
    generatedAt: questionResult.generatedAt || new Date().toISOString(),
    intentType: questionResult.intentType,
    primaryCount: (questionResult.primaryQuestions || []).length,
    optionalCount: (questionResult.optionalQuestions || []).length,
    questions: [
      ...(questionResult.primaryQuestions || []).map(q => ({ ...q, status: 'pending' })),
      ...(questionResult.optionalQuestions || []).map(q => ({ ...q, status: 'pending' })),
    ],
  };
  context.questionHistory.push(entry);
}

module.exports = { createContext, setIntent, addEntity, addPendingQuestion, answerQuestion, setContextVariable, getContextVariable, mergeContext, recordQuestionHistory };
