class SessionManager {
  ...
  create(userId, context = {}) {
    ...
    const session = {
      ...
      createdAt: now,
      lastAccessedAt: now,
      expiresAt: now + (context.maxLifetime || this._defaultMaxLifetime),
      idleTimeoutAt: now + (context.idleTimeout || this._defaultIdleTimeout),
  ...
  get(id) {
    ...
    if (Date.now() > session.expiresAt) { this._sessions.delete(id); return null; }
    return session;
  }
  ...
  validate(id) {
    ...
    if (Date.now() > session.idleTimeoutAt) {
      session.revoked = true;
      return { valid: false, error: 'Session idle timeout exceeded' };
    }
