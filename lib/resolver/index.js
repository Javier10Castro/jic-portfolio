...
async function resolveWorkspace({ workspace_slug, workspace_id }) {
  ...
  if (workspace_id) {
    ...
    sql = 'SELECT id, slug, plan, status FROM workspaces WHERE id = $1';
    params = [workspace_id];
  } else {
    sql = 'SELECT id, slug, plan, status FROM workspaces WHERE slug = $1';
  ...
  if (!r.rows.length) {
    ...
    throw makeError('NOT_FOUND', 'Workspace not found', field);
  }
