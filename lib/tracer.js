...
function _tryInitNeon() {
  ...
  try {
    ...
    console.error(JSON.stringify({ level: 'error', msg: 'tracer: failed to load neon module', error: e.message, stack: e.stack?.split('\n').slice(0,3).join('|') }));
    _neonReady = false;
  }
  return _neonReady;
}
...
module.exports = { trace, drain, getTraces, getCoverage, getExecutedPaths, ALL_PATHS };
