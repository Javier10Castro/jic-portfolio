...
async function drain() {
  ...
  if (count === 0) {
    console.log(JSON.stringify({ level: 'info', msg: 'tracer: drain called but nothing to drain', neonReady: _neonReady }));
    return;
  }
  const pending = _pending.splice(0);
  console.log(JSON.stringify({ level: 'info', msg: 'tracer: draining', count, neonReady: _neonReady }));
  const results = await Promise.allSettled(pending);
  const fulfilled = results.filter(r => r.status === 'fulfilled').length;
  const rejected = results.filter(r => r.status === 'rejected').length;
  if (rejected > 0) {
    console.error(JSON.stringify({ level: 'error', msg: 'tracer: drain completed with rejected promises', fulfilled, rejected }));
  }
}
